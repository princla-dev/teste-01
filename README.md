...
> **Nota:** O arquivo [`config.js`](./config.js) já está preenchido com as credenciais públicas (anon key) do projeto Supabase informado: `https://petvbrlseijabedxaspz.supabase.co`. Caso deseje usar outro projeto, basta atualizar esse arquivo com os valores correspondentes.

## Estrutura do projeto

```
├── background.js             # Service worker responsável por baixar os áudios
├── config.js                 # Configuração com URL e anon key do Supabase
├── config.example.js         # Exemplo de configuração para duplicar e editar
...
2. **Configuração local:**
   - Este repositório já inclui um `config.js` configurado com o projeto Supabase mencionado acima. Se preferir manter um repositório limpo para distribuição, você pode substituir o conteúdo por outro projeto ou até mesmo utilizar variáveis de ambiente durante o build.
   - Caso não configure o Supabase, o download continuará funcionando, mas o registro será ignorado e um aviso aparecerá no console.
...
3. Acesse uma página que contenha elementos `<audio>` ou `<source type="audio/...">` com URLs acessíveis.
4. Abra o popup da extensão e clique em **“Unir e baixar áudios”**.
5. Um arquivo `audios_unidos.wav` será baixado. Caso o Supabase esteja configurado, o evento ficará registrado na tabela `download_logs`.

## Limitações conhecidas

- Áudios servidos via URLs `blob:` ou streams dinâmicos não são capturados automaticamente. Será necessário adaptar o content script para interceptar `fetch`/`XHR` se o site trabalhar apenas com blobs em memória.
